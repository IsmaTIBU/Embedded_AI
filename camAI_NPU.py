...
class LEDClassificationNPU:
    ...
    def setup_model(self):
        """Configurar modelo con NPU si es posible, sino CPU"""
        try:
            print("Loading model on NPU")
            
            # Intentar NPU primero
            if self.try_load_npu():
                print("Npu successfully activated")
                self.processing_method = "NPU"
            else:
                print("No NPU support, falling back to CPU")
                self.load_cpu()
                self.processing_method = "CPU"
                
        except Exception as e:
            print(f"Error configurating model: {e}")
            self.load_cpu()
            self.processing_method = "CPU"
    ...
    def try_load_npu(self):
        ...
        try:
            ...
            if not os.path.exists(delegate_path):
                print(f"No delegate found: {delegate_path}")
                return False
    ...
    def setup_ui(self):
        ...
        self.window = Gtk.Window(title="NPU based classifier")
    ...
    def start_camera(self):
        ...
        # Actualizar status
        method_text = f"{self.processing_method}" if self.processing_method == "NPU" else f"{self.processing_method}"
    ...
    def update_ui(self, frame, result):
        ...
        try:
            ...
            self.performance_label.set_text(f"Performance: {result['inference_time']:.1f}ms ({self.processing_method})")
...
if __name__ == "__main__":
    print("=" * 50)
    print("NPU CLASSIFIER - i.MX 8M Plus")
